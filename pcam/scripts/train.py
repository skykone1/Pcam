...
from pcam.datasets.gd_dataset import GeoDesignDataset
...
def main(NB_EPOCHS, NUM_POINTS, RELOAD_LAST, DEVICE, PREFIX,
    ...
    if DATASET == 'kitti':
        ...
        # # modlenet40 train
        # train_loader = torch.utils.data.DataLoader(
        #     XJTU_Dataset(args=None, partition='train', dataset='modelnet40', subset='all', format='pytorch3d', pose_type='random', mask_type='render'),
        #     pin_memory=True, batch_size=1, collate_fn=m40.collate_fn, num_workers=4, shuffle=True
        # )
        # val_loader = torch.utils.data.DataLoader(
        #     XJTU_Dataset(args=None, partition='test', dataset='modelnet40', subset='all', format='pytorch3d', pose_type='random', mask_type='render'),
        #     pin_memory=True, batch_size=1, collate_fn=m40.collate_fn, num_workers=4, shuffle=False
        # )

        # # tudl-train
        # train_loader = torch.utils.data.DataLoader(
        #     XJTU_Dataset(args=None, partition='train', dataset='ycbv', mask_type='gt', format='bopdataset', pose_type='read_file'),
        #     pin_memory=True, batch_size=1, collate_fn=m40.collate_fn, num_workers=4, shuffle=True
        # )
        # val_loader = torch.utils.data.DataLoader(
        #     XJTU_Dataset(args=None, partition='test', dataset='ycbv', mask_type='gt', format='bopdataset', pose_type='re ad_file'),
        #     pin_memory=True, batch_size=1, collate_fn=m40.collate_fn, num_workers=4, shuffle=False
        # )

        # # tudl-finetune
        # train_loader = torch.utils.data.DataLoader(
        #     XJTU_Dataset(args=None, partition='train', dataset='tudl', subset='all', format='bopdataset', pose_type='random', mask_type='render'),
        #     pin_memory=True, batch_size=1, collate_fn=m40.collate_fn, num_workers=4, shuffle=True
        # )
        # val_loader = torch.utils.data.DataLoader(
        #     XJTU_Dataset(args=None, partition='test', dataset='tudl', subset='all', format='bopdataset', pose_type='random', mask_type='render'),
        #     pin_memory=True, batch_size=1, collate_fn=m40.collate_fn, num_workers=4, shuffle=False
        # )
        ...
        # BOP_DATASET = 'lm'
        train_loader = torch.utils.data.DataLoader(
            XJTU_Dataset_2(args=None, partition='train', dataset=BOP_DATASET, mask_type='gt', format='bopdataset', pose_type='read_file'),
            pin_memory=True, batch_size=1, collate_fn=m40.collate_fn, num_workers=4, shuffle=True)

        val_loader = torch.utils.data.DataLoader(
            XJTU_Dataset_2(args=None, partition='test', dataset=BOP_DATASET, mask_type='gt', format='bopdataset', pose_type='read_file'),
            pin_memory=True, batch_size=1, collate_fn=m40.collate_fn, num_workers=4, shuffle=False)


        # train_loader = torch.utils.data.DataLoader(GeoDesignDataset(args=None, partition='train'), 
        #     pin_memory=True, batch_size=1, collate_fn=m40.collate_fn, num_workers=4, shuffle=True)

        # val_loader = torch.utils.data.DataLoader(GeoDesignDataset(args=None, partition='test'), 
        #     pin_memory=True, batch_size=1, collate_fn=m40.collate_fn, num_workers=4, shuffle=False)


        # train_loader = torch.utils.data.DataLoader(
        #     XJTU_Dataset_2(args=None, partition='train', dataset='ycbv', mask_type='gt', format='bopdataset', pose_type='read_file'),
        #     pin_memory=True, batch_size=1, collate_fn=m40.collate_fn, num_workers=4, shuffle=True)

        # val_loader = torch.utils.data.DataLoader(
        #     XJTU_Dataset_2(args=None, partition='test', dataset='ycbv', mask_type='gt', format='bopdataset', pose_type='read_file'),
        #     pin_memory=True, batch_size=1, collate_fn=m40.collate_fn, num_workers=4, shuffle=False)
    ...
    # HUSH
    path2exp = root + '0323_trian_tudl_mid'
    ...
    if RELOAD_LAST:
        ...
        print('Loading PCAM/modelnet40_20epoch_best.pth')
        # checkpoint = torch.load(os.path.join(path2exp, 'check_point_last.pth'))
        checkpoint = torch.load('/home/hush/projects/PCAM/modelnet40_20epoch_best.pth')
