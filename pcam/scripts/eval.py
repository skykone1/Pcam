...
sys.path.append("/home/dingfei/projects/hush-PCAM/")
import pcam.datasets.modelnet40 as m40

from pcam.tool.test_final import test, test_bop
...
# from pcam.datasets.xjtu_dataset import XJTU_Dataset
from pcam.datasets.xjtu_dataset_2 import XJTU_Dataset_2
from pcam.datasets.kitti_dataset_2 import KittiDataset
from pcam.datasets.threedmatch_dataset_2 import ThreeDMatchDataset
...
def main(NUM_POINTS, DEVICE, THRESHOLD,
    ...
    checkpoint = torch.load('/home/hush/projects/PCAM/pcam/trained_models/modelnet/new_training/sparse_modelnet40_all/check_point_best.pth')
    # checkpoint = torch.load('/home/hush/projects/PCAM/pcam/trained_models/modelnet/new_training/sparse_modelnet40_all_tudl_3obj/check_point_best.pth')
    # checkpoint = torch.load('/home/hush/projects/PCAM/pcam/trained_models/modelnet/new_training/sparse_tudl_all/check_point_best.pth')
    
    # checkpoint = torch.load('/home/hush/projects/PCAM/pcam/trained_models/0313_trian_gd/check_point_best.pth')
    # checkpoint = torch.load('/home/hush/projects/PCAM/pcam/trained_models/0313_trian_tudl/check_point_best.pth')
    # checkpoint = torch.load('/home/hush/projects/PCAM/pcam/trained_models/0313_trian_lm/check_point_best.pth')
    # checkpoint = torch.load('/home/hush/projects/PCAM/pcam/trained_models/0321_trian_ycbv/check_point_best.pth')
    
    # checkpoint = torch.load('/home/hush/projects/PCAM/pcam/trained_models/0323_trian_tudl_mid/check_point_best.pth')
    # checkpoint = torch.load('/home/hush/projects/PCAM/pcam/trained_models/0323_trian_tudl_vox/check_point_best.pth')
    ...
    # ---
    # test_loader = torch.utils.data.DataLoader(m40.BOP_Dataset(), pin_memory=True, batch_size=1, num_workers=2, shuffle=False)

	# 3dmatch dataset
    test_root = "/cvlabdata1/hush/3dmatch/threedmatch_test"
    test_loader = torch.utils.data.DataLoader(ThreeDMatchDataset(test_root, "test", min_scale=1., max_scale=1., rotation_range=0, num_points=1024), # 1623
    ...
    # # kitti dataset
    # path2data = '/cvlabdata1/hush/kitti/dataset'
    # icp_cache_path = '/cvlabdata1/hush/kitti/icp'
    # test_loader = torch.utils.data.DataLoader(KittiDataset(path2data, "test", icp_cache_path, num_points=1024), # 555
    #                     pin_memory=True, batch_size=1,
    #                     # collate_fn=self.collate_fn,
    #                     num_workers=8, 
    #                     shuffle=False)
    ...
    # BOP_DATASET = 'lm'
    # BOP_DATASET = 'ycbv'
    # test_loader = torch.utils.data.DataLoader(XJTU_Dataset(args=None, partition='test', dataset=BOP_DATASET, mask_type='gt', format='bopdataset', pose_type='read_file'),
    #                         batch_size=1, num_workers=4, shuffle=False)

        
    # test_loader = torch.utils.data.DataLoader(XJTU_Dataset_2(args=None, partition='test', dataset=BOP_DATASET, mask_type='gt', format='bopdataset', pose_type='read_file'),
    #                         batch_size=1, num_workers=4,shuffle=False)

    
    test_bop(test_loader, net, DEVICE,
    ...
    # loader = torch.utils.data.DataLoader(XJTU_Dataset(args=None, partition='test', dataset='modelnet40', subset='all', format='pytorch3d', pose_type='random', mask_type='render'),
    #         pin_memory=True, batch_size=1, num_workers=4, collate_fn=m40.collate_fn, shuffle=False
