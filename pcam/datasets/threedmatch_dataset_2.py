...
from bpnet_utils.pytorch3d_utils import build_bijective_label
...
class ThreeDMatchDataset(PCAMDataset):
    ...
    def __getitem__(self, idx):
        ...
        R_gt, t_gt = T_gt[:3, :3], T_gt[:3, 3].reshape(3,1)

        label = build_bijective_label(unique_xyz0_th, unique_xyz1_th, R_gt, t_gt, self.voxel_size, 1)

        return unique_xyz0_th.T.astype('float32'), unique_xyz1_th.T.astype('float32'), \
                R_gt.astype('float32'), t_gt.astype('float32'), label, info_dict
