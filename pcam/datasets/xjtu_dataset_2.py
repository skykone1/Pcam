...
import sys
sys.path.append('/home/dingfei/projects/bop_toolkit')
from bop_toolkit_lib import inout

sys.path.append('/home/dingfei/projects/bpnet-22-new')
...
class XJTUBase(Dataset):
    ...
    def prepare_mesh(self, index):
        ...
        if self.dataset == 'modelnet40' :
            models_scale = 1 / 100 # just used to make the translation small.
        else:
...
class XJTU_Dataset_2(XJTUBase):
    ...
    def __getitem__(self, index: int):
        ...
        if self.mask_type == 'render':
            ...
            # if self.dataset not in ['modelnet40', 'xepose', 'tudl']:
                tgt = self.refine_mask_median(mask, depth, scene_camera, th=model_info['obj_size'].numpy())
                # tgt = self.refine_mask_median(mask, depth, scene_camera, th=1000.)
            else:
        ...
        # source, target = voxelization(pts1, pts2, self.args.th * model_info['obj_size'].numpy(), self.src_points, self.tgt_points)
        ...
        # label = build_bijective_label(source, target, R_gt, t_gt, self.args.th*model_info['obj_size'].numpy(), self.args.val, self.args.bi_layer)
...
def save_cache():
    ...
    args = Args()
    dataset = XJTU_Dataset_2(args=args, partition='test', dataset='xepose', mask_type='gt', format='bopdataset', pose_type='read_file')
    total = []
