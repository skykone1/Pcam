...
class BOP_Dataset(Dataset):
    def __init__(self):
        ...
        # self.xepose_cache = np.load('/home/hush/projects/PCAM/pcam/datasets/xepose_test_cache_1024768.npy', allow_pickle=True)
        # print('Xepose cache load complete.') 

        self.xepose_cache = np.load('/home/hush/projects/PCAM/pcam/datasets/tudl_test_targets_bop19.npy', allow_pickle=True)
        print('TUDL cache load complete.') 
        
        # self.xepose_cache = np.load('/home/hush/projects/PCAM/pcam/datasets/hb_test_cache_1024768.npy', allow_pickle=True)
        # print('HB cache load complete.') 

    def __len__(self):
        # return 4
        return len(self.xepose_cache)

    def __getitem__(self, index: int):
        return self.xepose_cache[index]['src'], self.xepose_cache[index]['tgt'], self.xepose_cache[index]['R_gt'], \
                self.xepose_cache[index]['t_gt'], 0, self.xepose_cache[index]['info_dict']


def test_ori_dataloader():

    train_loader = ModelNet40(1024, partition='train')

    for i in range(0, 10):
        xx = train_loader.__getitem__(i)
        print('1')
